...
This script expects an `estimators.py` file in the same directory that defines
`def BCM(X, Y, W, M) -> dict` returning a dict with keys 'est', 'se', 'AIse'.

Example estimators.py skeleton:

    # estimators.py
    import numpy as np
    def BCM(X, Y, W, M):
        # TODO: implement your estimator
        raise NotImplementedError("Implement BCM(X, Y, W, M) to return {'est':..., 'se':..., 'AIse':...}")
...
# Feather IO uses pyarrow under the hood
# pip install pandas pyarrow tqdm

try:
    from tqdm import tqdm
except Exception:  # pragma: no cover
    # fallback if tqdm isn't available
    def tqdm(x, **kwargs):
        return x
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Monte Carlo runner for BCM estimator (Python)")
    p.add_argument("--input_feather_path", type=str, default="data/exp_generated.feather",
                   help="Path to dataset containing treated + controls")
    p.add_argument("--output_feather_path", type=str, default="result/out.feather",
    ...
    # Where to store results. If provided, mc_runner will create a subfolder (run-name)
    # and write "out.feather" inside it. If not provided, we fall back to --output_feather_path.
    p.add_argument("--out-root", type=str, default=None, 
                   help="Results root. If set, write to <out-root>/<run-name or job-...>/out.feather")
    # Subdirectory name under out-root. If not provided, use job-$SLURM_JOB_ID or a timestamp.
    p.add_argument("--run-name", type=str, default=None, help="Subfolder name for this run (defaults to job-$SLURM_JOB_ID or a timestamp)")
...
# ---------------------------------------------
# Estimator registry (mirrors the R list)
# ---------------------------------------------
...
def BCM_ps(M: int, X: np.ndarray, Y: np.ndarray, W: np.ndarray) -> Dict[str, float]:
    """
    Wrapper around estimators.BCM that accepts:
    - dict with keys {'est','se','AIse'}
    - an object with attributes .est, .se, .AIse (e.g., MBCResult / dataclass)
    - a tuple/list/array of length >= 3 interpreted as (est, se, AIse)
    Returns a dict {'est': float, 'se': float, 'AIse': float}.
    """
    if BCM is None:
        raise RuntimeError("estimators.BCM not found. Create estimators.py with a BCM(X,Y,W,M) function.")
    out = BCM(X, Y, W, M)
    ...
    if isinstance(out, dict):
        try:
            ...
            raise ValueError(f"BCM dict output must have keys 'est','se','AIse'. Got keys: {list(out.keys())}") from e

    # Case 2: object with attributes (e.g., MBCResult dataclass)
    if hasattr(out, "est") and hasattr(out, "se") and hasattr(out, "AIse"):
        return {"est": float(out.est), "se": float(out.se), "AIse": float(out.AIse)}

    # Case 3: tuple / list / array
    if isinstance(out, (tuple, list, np.ndarray)) and len(out) >= 3:
        est, se, AIse = out[:3]
        return {"est": float(est), "se": float(se), "AIse": float(AIse)}

    raise TypeError(
        "Unsupported BCM return type. Expected dict with keys {'est','se','AIse'}, "
        "or an object with attributes .est/.se/.AIse, or a 3-tuple (est, se, AIse). "
        f"Got type: {type(out)}"
    )
...
def get_sample(df: pd.DataFrame, covariates: List[str], N_treated: int, N_control: int, rng: np.random.Generator) -> Sampled:
    ...
    Y = s["re78"].to_numpy(copy=False)
    W = s["t"].to_numpy(copy=False)
    return Sampled(X=X, Y=Y, W=W)



# ---------------------------------------------
# Bootstrap utilities: percentile CI 
# ---------------------------------------------
...
def bootstrap_percentile_CIs(
    ...
    - keep N1 and N0 fixed; resample indices within treated/control groups separately.
    - This is crucial for valid coverage in this design.
    """
    if hat_est is None:
    ...
    # independent child RNG for bootstrap draws
    child = np.random.default_rng(rng.integers(0, 2**63 - 1, dtype=np.int64))

    tau_stars = np.empty(B, dtype=float)
    for b in range(B):
        # resample within groups
        j1 = child.choice(idx1, size=n1, replace=True)
        j0 = child.choice(idx0, size=n0, replace=True)
        jb = np.concatenate([j1, j0])
...
def main():
    ...
    alphas = parse_alpha_list(args.alphas)           # e.g., [0.10, 0.05]
    alpha_tags = [alpha_to_tag(a) for a in alphas]   # -> ['p90','p95']

    # NEW: parse M multipliers
    M_poly_list = [float(s) for s in str(getattr(args, "M_poly_list", "0.5,1,2,5")).split(",") if s.strip()]

    # Normalize input path (relative -> absolute) and validate existence
    ...
    # Decide final output location
    if getattr(args, "out_root", None):
        out_root = Path(args.out_root)
        if not out_root.is_absolute():
            out_root = Path.cwd() / out_root

        # Choose run-name: prefer user-specified, else SLURM job id, else timestamp
        ...
        # Canonical output file inside the run folder
        args.output_feather_path = str(out_dir / "out.feather")
    else:
        # Legacy mode: respect --output_feather_path (make absolute + ensure parent)
        out_path = Path(args.output_feather_path)
        if not out_path.is_absolute():
            out_path = Path.cwd() / out_path
        out_path.parent.mkdir(parents=True, exist_ok=True)
        args.output_feather_path = str(out_path)
        out_dir = out_path.parent  # handy for any extra files
    ...
    required_cols = set(["t", "re78"]) | set(COVARIATES)
    ...
    # We only have one estimator method name:
    method_names = ["BCM_ps"]

    print(f"starting iterations with input {args.input_feather_path}")

    iter_counter = 0
    # Fixed treated:control ratio = 185 : 260
    p_treated = 185 / (185 + 260)
    ...
    for N in N_list:
        ...
        for c in M_poly_list:
            ...
            for method in method_names:
                # Count how many already completed for this (N, M, method)
                mask = (
                ...
                while N_completed < args.N_runs:
                    # Each chunk runs N_workers * N_chunk iterations
                    N_tasks = min(max(1, args.N_workers) * args.N_chunk, max(0, args.N_runs - N_completed))

                    # Use a base seed that changes each loop but is reproducible wrt (N,M,method,iter)
                    ...
                    # Re-read existing results if resuming, to be robust against external writers
                    if (args.resume and os.path.exists(args.output_feather_path)) or (iter_counter > 0):
                        try:
                            runs_df = pd.read_feather(args.output_feather_path)
                        except Exception:
                            # If read fails, keep current in-memory df
                            pass
